...
import { Popover, Form, Button, Input, Slider, Radio } from 'antd';
		...
		document.querySelectorAll("#toolContainer [role = 'toolItem']").forEach((item, index) => {
			...
			toolDraggie.on('dragEnd', function (event, pointer) {
				...
				if (that.isOverlap(item, document.getElementById('contentContainer'))) {
					...
					let son = that.state.activeList.length - 1

					// 同步修改state中的位置值
					activeDraggie.on('dragEnd', function () {
						that.state.activeList[son].x = activeDraggie.position.x;
						that.state.activeList[son].y = activeDraggie.position.y;
					})
				}
